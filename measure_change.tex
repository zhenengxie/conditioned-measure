...
\begin{lemma}
    ...
    \begin{equation}
        \Pr_n\left( \Delta_{n-m} = \sum_{i=1}^m (k^+_i - k^-_i) \right) 
        \sim \Pr(\Delta_n = 0)
        \sim \frac{1}{\sqrt{2 \pi \diffsd^2 n}}
    \end{equation}
    as $n \to \infty$.
\end{lemma}

When expanding the measure change, instead of expanding in terms of $\Xi^-_{n-m}$ it is convenient to expand in terms of $\Omega^-_n$ which is almost $\Pr_n$-centered. The following lemma provides probabilistic bounds on $\Omega_n^-$.

\begin{lemma}
    Define
    \begin{equation}
        \Omega_n^- \defeq \Xi^-_{n-m} - \mu n + \nu_- m.
    \end{equation}
    Then
    \begin{equation}
        \lim_{n \to \infty} \Pr_n\left( \abs{\Omega^-_n} \leq n^{\frac{1}{2} + \epsilon} \Biggm| \Delta_{n-m} = \sum_{i=1}^m (k^+_i - k^-_i) \right) = 1.
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    Finally
    \begin{equation}
        \frac{\Pr_n(\omegaevent\cap A_n)}{\Pr(\Delta_n = 0)} = \Pr_n(\omegaevent \mid A_n) \frac{\Pr_n(A_n)}{\Pr(\Delta_n = 0)} \to 1
    \end{equation}
    as $n \to \infty$ by INSERT REFERENCE and INSERT REFERENCE which gives the desired final result.
\end{proof}
